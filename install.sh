...
REPO_URL="https://github.com/roger8b/arxiv-search-cli"
INSTALL_DIR="${HOME}/.arxiv-cli"
USE_LOCAL=false
LOCAL_SOURCE=""
RUN_SETUP=true
...
Usage: $0 [--local [src]] [--no-setup]
  ...
  --local [src]   Use local checkout instead of cloning. If 'src' given,
                  syncs from that path into ~/.arxiv-cli/.
  --no-setup      Skip 'arxiv setup' at the end.
EOF
                exit 0 ;;
...
# Verify arxiv is on PATH
if ! command -v arxiv >/dev/null 2>&1; then
  NPM_PREFIX="$(npm config get prefix 2>/dev/null || echo '')"
  warn "arxiv not on PATH"
  dim "npm bin dir: ${NPM_PREFIX}/bin"
  dim "add to your shell rc:  export PATH=\"${NPM_PREFIX}/bin:\$PATH\""
fi

ok "arxiv installed ($(arxiv --version 2>/dev/null || echo 'ok'))"

if $RUN_SETUP; then
  echo ""
  dim "running: arxiv setup"
  echo ""
  arxiv setup || warn "setup did not complete — run 'arxiv setup' manually when ready"
fi
...
echo ""
echo "  Try:"
echo "    arxiv \"machine learning transformers\""
echo "    arxiv --help"
echo ""
